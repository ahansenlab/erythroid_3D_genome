...
def test_sweep(test_idx, region_size, CTCF_L, CTCF_R, sticky_elts, birthArray, deathArray):
    """
    ...
    death_slice = deathArray[test_region_start:test_region_end]
    death_slice = death_slice / sum(death_slice)  # Normalize
    ...
    axes[0].stem(x, ctcf_r_slice, linefmt='r-', markerfmt='ro', basefmt='k-', use_line_collection=True)
    ...
    axes[3].plot(x, death_slice, color='purple', drawstyle='steps-mid')
    axes[3].fill_between(x, 0, death_slice, color='purple', alpha=0.3, step="mid")
    axes[3].set_ylabel('Lifetime')
    # axes[2].set_ylim([0, 0.01])
    # We let matplotlib auto-scale the Y-axis here because if birthArray
    # was already normalized (birthArray / sum), the values will be tiny.
...
if __name__ == '__main__':
    ...
    for i, config in enumerate(generate_param_grid(simulation_sweep_space)):
        ...
        pause_prob = 1 - 0.0025
        base_death_rate = 1. / (0.5 * config['processivity'] / (1 - pause_prob))
        deathArray = np.zeros(chrom_size, dtype=np.double) + base_death_rate

        if len(boundary_coords) > 0:
            deathArray[boundary_coords] = base_death_rate / config['ctcf_boost_factor']
        ...
        if loading_loc_biases is None:
            loading_loc_biases = np.ones(chrom_size)
        fig = test_sweep(0, region_size, stall_L, stall_R, monomer_types, loading_loc_biases, deathArray)
        plt.show()
        #fig.savefig(os.path.join(savedir, f"params_{i}.svg"))
        #fig;