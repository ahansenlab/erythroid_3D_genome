...
control="$1"
test="$2"
database="${3:-/mnt/md0/varshini/Analysis/Motifs/JASPAR2024_CORE_vertebrates_non-redundant_pfms_meme}"
outdir="${4:-/mnt/md0/varshini/Analysis/Blood/complete_analyses/boundaries/motif_enrichment}"
...
./run_ame_merged.sh "${control}_atac.fa" "${test}_atac.fa" "${outdir}/motif_output_anchors_atac_${btest}_over_${bcontrol}" "$database"
#./run_ame_merged.sh "${control}.fa" "${test}.fa" "${outdir}/motif_output_anchors_${btest}_over_${bcontrol}" "$database"
